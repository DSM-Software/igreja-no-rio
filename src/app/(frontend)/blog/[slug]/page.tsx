...
export default async function PostPage({ params }: Props) {
  ...
  const coverImage =
    post.coverImage && typeof post.coverImage === 'object'
      ? (post.coverImage as any)?.sizes?.hero?.url ?? (post.coverImage as any)?.url
      : null
  ...
  return (
    <article>
      ...
      <div className="container" style={{ maxWidth: 780, paddingBlock: 'clamp(40px, 6vh, 72px)' }}>
        {/* Category / serie tags */}
        <div style={{ display: 'flex', gap: 8, marginBottom: 24 }}>
          <span className={`tag tag-${post.coverColor === 'navy' ? 'navy' : 'teal'}`}>{post.category}</span>
          {post.serie && (
            <span className="tag tag-neutral">{post.serie} · Parte {post.serieParte}</span>
          )}
        </div>
        ...
        {post.tags && post.tags.length > 0 && (
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 48 }}>
            {post.tags.map((t: { tag?: string | null; id?: string | null }) => (
              <span key={t.tag} className="tag tag-neutral">{t.tag}</span>
            ))}
          </div>
        )}
        ...
        {seriesPosts.length > 1 && (
          ...
          >
            <p style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 13, letterSpacing: '.08em', textTransform: 'uppercase', color: 'var(--muted)', marginBottom: 16 }}>
              Série: {post.serie}
            </p>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 16 }}>
              {prevPost ? (
                <Link href={`/blog/${prevPost.slug}`} className="btn btn-outline btn-sm">
                  ← Parte {prevPost.serieParte}: {prevPost.title}
                </Link>
              ) : <span />}
              {nextPost && (
                <Link href={`/blog/${nextPost.slug}`} className="btn btn-primary btn-sm">
