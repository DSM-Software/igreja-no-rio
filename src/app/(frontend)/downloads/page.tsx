...
export const metadata: Metadata = {
  title: 'Downloads',
  description: 'Pregações em áudio, PDFs de estudo e slides dos grupos caseiros da Igreja no Rio.',
...
export default async function DownloadsPage() {
  ...
  const categoryOrder = ['Pregações', 'Estudos', 'Grupos Caseiros', 'Devocionais']
      ...
      <div className="page-hero" style={{ paddingTop: 'calc(var(--nav-h) + 48px)' }}>
        <div className="container">
          ...
          <p className="section-desc" style={{ marginTop: 16 }}>
            Áudios das pregações, apostilas de estudo, roteiros de grupos caseiros e devocionais para levar para casa.
          </p>
        </div>
      </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 56 }}>
              {sortedCategories.map((cat) => (
                <section key={cat}>
                  ...
                  >
                    {cat}
                    <span style={{ marginInlineStart: 8, fontWeight: 400, fontSize: 14, color: 'var(--muted)' }}>
                      ({groups[cat].length})
                    </span>
                  </h2>
