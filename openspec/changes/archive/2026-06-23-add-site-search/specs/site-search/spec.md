...
O sistema SHALL expor o endpoint público `GET /api/search?q=<termo>&limit=<n>&offset=<n>&type=<posts|events|all>` que SHALL retornar JSON `{ posts: SearchHit[], events: SearchHit[], total: number }`, com top-N por tipo e cabeçalho `Cache-Control: public, max-age=60, stale-while-revalidate=300`.
...
- **THEN** os seguintes fluxos passam: (a) gatilho visível no header em desktop e mobile; (b) atalho `/` abre overlay; (c) digitar 2+ letras dispara busca e exibe agrupamento; (d) navegação por setas + Enter abre o hit; (e) estado "sem resultados" aparece para termo improvável; (f) página `/busca?q=...` carrega com `noindex`; (g) post `published: false` no seed não aparece nos resultados
