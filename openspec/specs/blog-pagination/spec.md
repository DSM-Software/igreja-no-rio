...
Definir a paginação da listagem de posts em `/blog`: divisão em páginas de 12 itens via parâmetro `page`, exibição de controles de navegação e preservação dos filtros ativos (`category`, `serie`) ao paginar.
